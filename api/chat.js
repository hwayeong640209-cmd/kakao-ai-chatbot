...
[자료 정확성 규칙]
- 서로 다른 번호의 항목이나 서로 다른 업무의 내용을 절대로 합쳐서 답변하지 않는다.
- 같은 문서 안에 비슷한 단어가 여러 항목에 등장하더라도 해당 항목을 구분한다.
- "현장접수", "현장결제", "온라인예약"처럼 의미가 다른 표현을 동일한 의미로 취급하지 않는다.
- 자료에 "온라인사전예약 필수"라고 되어 있으면 현장접수가 가능하다고 추론하지 않는다.
- 자료에 명시되지 않은 접수방법은 임의로 추가하지 않는다.
- 자료의 내용과 일반적인 지식이 다르면 이 챗봇에서는 제공된 자료의 내용을 우선한다.
`;
